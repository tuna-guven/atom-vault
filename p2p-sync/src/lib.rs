...
impl AtomSyncManager {
    ...
    pub async fn connect_to_friend(
        ...
        master_secret: &[u8; 32],
    ) -> Result<(transport::Control, tokio::sync::mpsc::Receiver<yamux::Stream>), Box<dyn std::error::Error>> {
        ...
        // 1. Boot up the embedded Tor Client
        // This downloads the consensus and builds circuits. In a real app, you would 
        // initialize this once at startup and share the Arc<TorClient> across connections.
        let config = TorClientConfig::default();
        println!("Bootstrapping onto the Tor network...");
        let tor_client = TorClient::create_bootstrapped(config).await?;

        // 2. Dial the .onion address directly over the Tor network
        let arti_stream = tor_client.connect((onion_addr.as_str(), 80)).await?;
        
        // Convert Arti's Futures-based stream into a Tokio-compatible stream
        let mut stream = arti_stream.compat();

        // 3. Execute the Cryptographic Handshake
        let session = handshake::execute_handshake(
            &mut stream,
            true, 
            &self.local_identity,
            friend_identity,
        ...
        // 4. Initialize the Multiplexer
        let (yamux_control, inbound_rx) = transport::start_multiplexer(
            stream,
            session.transport,
            true, 
        );
