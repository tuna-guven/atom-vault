...
// Whitelist base system paths (Read Only)
    // /tmp dizini OS güvenliği gereği sadece okuma izniyle bırakıldı.
    let system_read_access = AccessFs::ReadFile | AccessFs::ReadDir | AccessFs::Execute;
    let system_paths = ["/usr", "/lib", "/lib64", "/etc", "/var/cache/fontconfig", "/tmp", "/sys"];
    ...
    // --- KRİTİK GÜNCELLEME: Geçici Alan (Volatile) Yaratma, Silme ve Kesme İzinleri ---
    // SQLite'ın WAL dosyalarını ve GTK'nın geçici dosyalarını yönetebilmesi için tam set
    ...
    if let Ok(xdg_runtime) = env::var("XDG_RUNTIME_DIR") {
        ...
        // GTK requires dconf cache (living in RAM)
        ...
        // Zathura için geçici veritabanı alanını TAM YETKİ (volatile_rw_access) ile beyaz listeye ekle
    ...
    // 2. Read-Only GTK Themes (Prevents UI from breaking, zero security risk)
    ...
    // TMPDIR EKLENDİ: Zathura gerçek /tmp'ye dokunamayacak, sadece sahte RAM alanına yazacak
    ...
    unsafe {
        match libc::fork() {
            0 => {
                ...
                // Error logging for sandbox debugging
                let current_dir = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
                let log_path = current_dir.join("zathura_debug.log");
                let err_log = CString::new(log_path.to_str().unwrap()).unwrap();
                ...
                // Rewind pipe to prevent format recognition errors in PDF parser
